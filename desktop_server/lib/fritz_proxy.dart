...
/// Standalone version — no Flutter dependency, reads HTML from disk.
class FritzProxy {
  HttpServer? _server;
  ...
  FritzProxy({int port = 8742}) : _requestedPort = port;
  ...
  Future<void> start() async {
    ...
    // Serve the HTML UI from disk
    router.get('/', (shelf.Request request) async {
      _cachedHtml ??= await _loadHtml();
      return shelf.Response.ok(_cachedHtml,
          headers: {'Content-Type': 'text/html; charset=utf-8'});
    });

    // Return the device's local network IPs
    ...
    // BSSID not available on desktop — return null (JS handles gracefully)
    router.get('/wifi-bssid', (shelf.Request request) async {
      return _jsonResponse({'bssid': null, 'error': 'Not available on desktop'});
    });
  ...
  /// Load HTML from disk, searching several candidate paths
  Future<String> _loadHtml() async {
    final candidates = [
      'assets/fritzbox_tools.html',
      '../flutter_app/assets/fritzbox_tools.html',
      'fritzbox_tools.html',
    ];
    ...
    throw FileSystemException('Could not find fritzbox_tools.html. '
        'Run from desktop_server/ or place the HTML in assets/');
  }
