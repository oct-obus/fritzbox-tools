...
## 11. Settings Backup (.export) File Format

### Overview
Fritz!Box `.export` files are UTF-8 text files with structured sections. Generated by `libcfgimpexp.so.0.0.0` (source: `foslibs/high/cfgimpexp/`).

### Structure
```
**** FRITZ!Box 7590 CONFIGURATION EXPORT
Password=<32 hex chars>          ← hex-encoded AES IV (NOT a password hash)
FirmwareVersion=08.25
OEM=avm
Country=049
Language=de

**** CFGFILE:/var/flash/ar7.cfg
<plaintext config key=value pairs>
/*ImportKey=<md5_hex>*/

**** CRYPTEDB64FILE:/var/flash/voip.cfg
<base64(AES-256-CBC(plaintext))>

**** END OF FILE ****
**** END OF EXPORT XXXXXXXX ****     ← CRC32 of entire content
```

### Section Types
- `CFGFILE:<path>` — plaintext key=value config
- `B64FILE:<path>` — binary config, base64-encoded
- `BINFILE:<path>` — raw binary config
- `CRYPTEDB64FILE:<path>` — AES-256-CBC encrypted + base64
- `CRYPTEDBINFILE:<path>` — AES-256-CBC encrypted, raw

### Encryption

**Password-protected exports (CRYPTEDB64FILE/CRYPTEDBINFILE):**
- Algorithm: AES-256-CBC (via OpenSSL EVP)
- Key: `MD5(user_password)` → 16 bytes, zero-padded to 32 bytes
- IV: 16 random bytes, hex-encoded in the `Password=` header field
- No salt, no iterations — raw single MD5
- Effective security: 128-bit (despite AES-256) due to MD5 output size

**No-password exports:**
- Same AES-256-CBC scheme
- Default password hardcoded in `libcfgimpexp.so` rodata: `Xy!9>5fkv8f:-?vfv`
- Key = `MD5("Xy!9>5fkv8f:-?vfv")` — constant across all Fritz!Box devices

**avmnexus/boxcert sections:**
- Algorithm: AES-128-CBC via `libavmcipher`
- Key: hardcoded 16 bytes: `71 77 65 72 61 73 64 78 76 6c fc 30 33 6e 63 00`
- HMAC-SHA1 integrity check on ciphertext
- Same key on every Fritz!Box

### Files Included
ar7.cfg, wlan.cfg, voip.cfg, user.cfg, vpn.cfg, broadband.cfg, tr069.cfg, tr369.cfg, autodetect.cfg, dect_misc, dect_eeprom, phonebook, calllog, tamconf, configd, avmnexus.cfg, plc.pib, and more from `/var/flash/`.

Passwords within CFGFILE sections are scrubbed during export (fields: entry, pass, uipin, pin, Value, ppp_password*).

### Security Weaknesses
1. MD5 key derivation — no salt, no iterations (~10B hashes/sec on GPU)
2. Hardcoded default password `Xy!9>5fkv8f:-?vfv` — all no-password exports use the same key
3. Hardcoded AES-128 key for avmnexus/boxcert — firmware analysis reveals it
4. CRC32 footer — not a cryptographic MAC, tampering undetected if recomputed
5. AES-256 with 128-bit effective key (MD5 output zero-padded)
...
### Critical
- **`post.lua` — SID check commented out**: `--require"check_sid"` → time-setting endpoint works without session from LAN
- **REST API security kill-switch**: `webui:settings/rest_api_endpoint_security=0` disables all REST API auth
- **Hardcoded chrony password**: `avm` literal in `etc/init.d/rc.chrony`
- **TFFS plaintext storage**: All credentials in `/var/flash/` stored without encryption
- **Factory ptest suite in consumer firmware**: `rc.ptest.sh` with SetFactoryDefaults, SwitchFirmware etc.

### Medium
- **UPnP AddPortMapping**: No auth, any LAN device can create WAN port forwards
- **IPSec legacy ciphers**: MD5, SHA-1, 3DES, MODP-1536 in default profiles
- **Passwordless user auto-login**: Empty password + isStandardFritzUser = auto LAN auth
- **UART root shell**: `ttyLTQ0::askfirst:-/bin/sh` — no password on serial console

### Info
- Internal AVM GitLab URL in production (`gitlab.avm.de/fos/networking.source`)
- Developer note in `system_status` asking colleagues to leave their logo
- `rc.ptest.release` is wrong placeholder file
- Internal AVM IPs `192.168.116.252/253` in avme ipsec.cfg
- Weather telemetry to `api.avm.de` with area codes
- Kernel 4.9.337 + GCC 8.4.0 (both EOL)
